...
import {
  FiUser, FiActivity, FiCalendar, FiMessageSquare, FiClock,
  FiCheckCircle, FiUsers, FiArrowRight, FiHeart, FiBookOpen,
  FiMessageCircle, FiTrendingUp, FiList, FiChevronLeft, FiChevronRight,
} from 'react-icons/fi';
...
const Dashboard = () => {
  ...
  // Load bookings, events, peer sessions
  useEffect(() => {
    if (!user?._id) return;
    const load = async () => {
      try {
  ...
  // Load upcoming confirmed sessions
  useEffect(() => {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="container-custom -mt-10 pb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <StatCard icon={FiCalendar} label="Total Bookings" value={bookings.length} sub="All time" iconBg="bg-primary-50" iconColor="text-primary-600" />
          <StatCard icon={FiMessageCircle} label="Peer Sessions" value={sessions.length} sub="With peer counselors" iconBg="bg-green-50" iconColor="text-green-600" />
          <StatCard icon={FiCheckCircle} label="Completed" value={completed.length} sub="Sessions done" iconBg="bg-purple-50" iconColor="text-purple-600" />
        </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <Link to="/booking/my" className="text-primary-600 text-sm hover:text-primary-700 flex items-center gap-1 font-medium">
              ...
              {bookings.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                  ...
                  <Link to="/events" className="mt-4 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm rounded-lg font-medium transition-colors">
                    Browse Events
                  </Link>
                </div>
              ) : (
                <div className="space-y-2.5">
                  {bookings.slice(0, 5).map((b) => (
                    <div key={b._id} className="flex items-center gap-3 p-3.5 rounded-xl bg-gray-50 hover:bg-slate-100 transition-colors">
                      <div className="w-10 h-10 rounded-xl bg-primary-100 flex items-center justify-center flex-shrink-0">
                        <FiCalendar className="w-4 h-4 text-primary-600" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-900 text-sm truncate">{b.eventId?.title || 'Session'}</p>
                        <p className="text-xs text-gray-400 mt-0.5">{fmtDate(b.eventId?.startDate)}</p>
                      </div>
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full capitalize flex-shrink-0 ${STATUS_STYLES[b.status] || 'bg-gray-100 text-gray-600'}`}>
                        {b.status}
                      </span>
                    </div>
                  ))}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <Link to="/my-sessions" className="text-primary-600 text-sm hover:text-primary-700 flex items-center gap-1 font-medium">
                  View all <FiArrowRight className="w-3.5 h-3.5" />
                </Link>
              </div>
              {sessions.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-gray-400">
                  ...
                  <Link to="/peer-supporters" className="mt-4 px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm rounded-lg font-medium transition-colors">
                ...
                <div className="space-y-2.5">
                  {sessions.slice(0, 5).map((s) => {
                    ...
                    return (
                      <Link key={s._id} to="/my-sessions" className="flex items-center gap-3 p-3.5 rounded-xl bg-green-50 hover:bg-green-100 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-gray-900 text-sm truncate">{peer?.name || 'Peer Counselor'}</p>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="bg-gradient-to-br from-violet-500 to-purple-600 rounded-2xl p-5 text-white shadow-sm">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-purple-200">Daily Tip</span>
              </div>
              ...
              <div className="mt-4 flex gap-1">
                {WELLNESS_TIPS.map((_, i) => (
                  <div
                    key={i}
                    className={`h-1 rounded-full flex-1 ${i === new Date().getDay() % WELLNESS_TIPS.length ? 'bg-white' : 'bg-white/25'}`}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="space-y-0.5">
                ...
                <QuickActionLink to="/content-library" icon={FiBookOpen} label="Content Library" iconBg="bg-indigo-50" iconColor="text-indigo-600" />
                <QuickActionLink to="/emergency-contacts" icon={FiUsers} label="Emergency Contacts" iconBg="bg-red-50" iconColor="text-red-600" />
                  ...
                  upcomingSessions.map((session, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left transition-colors ${isTimeAvailable ? 'hover:bg-gray-50 cursor-pointer' : 'cursor-not-allowed opacity-60'}`}
        ...
        {events.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <Link to="/events" className="text-primary-600 text-sm hover:text-primary-700 flex items-center gap-1 font-medium">
                See all <FiArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {events.map((e) => (
                <Link key={e._id} to={`/events/${e._id}`} className="group rounded-xl border border-gray-100 overflow-hidden hover:shadow-md transition-all hover:-translate-y-0.5">
                  ...
                  <div className="p-4">
                    <p className="font-semibold text-gray-900 text-sm line-clamp-1 group-hover:text-primary-600 transition-colors">{e.title}</p>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <span className="text-xs text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full">{e.seatsAvailable} left</span>
                    </div>
                  </div>
                </Link>
              ))}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => scrollSlider(-1)} className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center hover:bg-gray-50 transition-colors">
          ...
          <div ref={sliderRef} className="flex gap-4 overflow-x-auto pb-2 scroll-smooth" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {COMMUNITY_GROUPS.map((group) => (
              <Link key={group.id} to="/peer-supporters" className={`flex-shrink-0 w-56 rounded-xl border p-4 hover:shadow-md transition-all hover:-translate-y-0.5 cursor-pointer ${group.bg} ${group.border}`}>
                ...
                <div className="flex items-center justify-between">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full bg-white/70 ${group.iconColor}`}>{group.tag}</span>
          ...
          <div className="bg-gradient-to-r from-primary-50 to-sky-50 border border-primary-100 rounded-2xl p-6 flex items-start gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                <Link to="/events" className="px-3 py-1.5 bg-primary-600 hover:bg-primary-700 text-white text-xs rounded-lg font-medium transition-colors">
                  Explore Events
                </Link>
                <Link to="/profile" className="px-3 py-1.5 bg-white hover:bg-primary-50 text-primary-700 border border-primary-200 text-xs rounded-lg font-medium transition-colors">
                  Complete Profile
                </Link>
              </div>
            </div>
